// Global DOM refs used across calculator functions (assigned in setup)
let electricityCalculator, resultsContainer, calculateBtn, categoryBtns;
    ...
    function calculateFuelEmissions() {
        ...
        const factors = {
            ...
            Activa: { fuelPerHour: 0.08, ef: 1.51, unit: "kg" }
        };
...
const APPLIANCES = {
    ...
    fan: { name: 'Ceiling Fan', watt: 70, defaultHours: 5 },
    ...
    laptop: { name: 'Laptop', watt: 60, defaultHours: 5 },
    desktop: { name: 'Desktop Computer', watt: 200, defaultHours: 4 },
    router: { name: 'WiFi Router', watt: 10, defaultHours: 24 },
    chargers: { name: 'Phone Charger', watt: 5, defaultHours: 5 },
    mixer_grinders: { name: 'Mixer/Grinder', watt: 750, defaultHours: 1 },
    electric_chimney: { name: 'Electric Chimney', watt: 300, defaultHours: 1 },
    coolers: { name: 'Cooler', watt: 150, defaultHours: 5 },
    lamp: { name: 'Lamp', watt: 60, defaultHours: 5 }
...
function setup() {
    ...
    // Get DOM elements (assign to globals)
    categoryBtns = document.querySelectorAll('.category-btn');
    electricityCalculator = document.getElementById('electricityCalculator');
    ...
    if (categoryBtns) {
        categoryBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // Remove any product prompt containers to avoid leftover buttons when switching categories
                ['productPromptContainer', 'buildingProductPrompt', 'waterProductPrompt'].forEach(id => { const el = document.getElementById(id); if (el) el.remove(); });

                // Show appropriate calculator immediately
                switch(category) {
                    case 'electricity':
    ...
    function buildElectricityInputs() {
        ...
        const groups = {
            ...
            'Cooling': {
                icon: '❄️',
                items: ['fan', 'ac'  , 'coolers']
            },
            'Kitchen': {
                icon: '🍳',
                items: ['fridge', 'microwave', 'induction', 'geyser', 'mixer_grinders', 'electric_chimney']
            },
            'Electronics': {
                icon: '🖥️',
                items: ['tv', 'laptop', 'desktop', 'router' , 'chargers']
            }
        };
    ...
    function calculateBuildingScore() {
        ...
        // Require user to answer at least one question before calculating
        let answeredAny = false;
        questions.forEach(q => {
            const yesRadio = document.querySelector(`input[name='${q}'][value='yes']`);
            const noRadio = document.querySelector(`input[name='${q}'][value='no']`);
            if ((yesRadio && yesRadio.checked) || (noRadio && noRadio.checked)) answeredAny = true;
            if (yesRadio && yesRadio.checked) score++;
        });

        if (!answeredAny) {
            resultsContainer.innerHTML = `
                <div class="results-section" style="text-align:center; padding:1.5rem;">
                    <p style="color:#c62828;">Please answer at least one question before viewing the building score.</p>
                </div>
            `;
            return;
        }
    ...
    // Remove other product containers (electricity/water) to avoid duplicates
    const removeIdsBeforeBuilding = ['productPromptContainer', 'waterProductPrompt'];
    removeIdsBeforeBuilding.forEach(id => { const el = document.getElementById(id); if (el) el.remove(); });

    // Insert a "View related products" button for building category
    const buildingContainerId = 'buildingProductPrompt';
    const existingBuilding = document.getElementById(buildingContainerId);
    if (existingBuilding) existingBuilding.remove();
        const buildingContainer = document.createElement('div');
        buildingContainer.id = buildingContainerId;
        buildingContainer.style.marginTop = '1rem';
        buildingContainer.innerHTML = `<a href="products.html?cat=building" class="btn btn-primary">View related products</a>`;
        if (resultsContainer && resultsContainer.parentNode) {
            resultsContainer.parentNode.insertBefore(buildingContainer, resultsContainer.nextSibling);
        }
    ...
    function calculateWaterEmissions() {
        ...
        // If user hasn't entered an amount, don't calculate yet
        if (!amount || amount <= 0) {
            resultsContainer.innerHTML = `
                <div class="results-section" style="text-align:center; padding:1.5rem;">
                    <p style="color:#c62828;">Please enter a water amount before calculating emissions.</p>
                </div>
            `;
            return;
        }
    ...
    // Remove other product containers (electricity/building) to avoid duplicates
    const removeIdsBeforeWater = ['productPromptContainer', 'buildingProductPrompt'];
    removeIdsBeforeWater.forEach(id => { const el = document.getElementById(id); if (el) el.remove(); });

    // Insert a "View related products" button below tips (for water category)
    const waterContainerId = 'waterProductPrompt';
    const existingWater = document.getElementById(waterContainerId);
    if (existingWater) existingWater.remove();
        const waterContainer = document.createElement('div');
        waterContainer.id = waterContainerId;
        waterContainer.style.marginTop = '1rem';
        waterContainer.innerHTML = `<a href="products.html?cat=water" class="btn btn-primary">View related products</a>`;
        if (resultsContainer && resultsContainer.parentNode) {
            resultsContainer.parentNode.insertBefore(waterContainer, resultsContainer.nextSibling);
        }
    ...
    let efficiencyScore = 100 - ((totalKwh / averageMonthlyKwh) * 10);
    if (efficiencyScore < 10) efficiencyScore = 10;

    if (efficiencyScore < 50) efficiencyScore = 50;
    ...
    efficiencyScore = Math.round(efficiencyScore);
        ...
        // If bill estimator is present, prefill units with calculated monthly kWh
        const billUnitsEl = document.getElementById('billUnits');
        if (billUnitsEl) {
            billUnitsEl.value = Number(totalKwh.toFixed(2));
        }

    // Remove other product containers (building/water) to avoid duplicates
    const removeIdsBeforeElectric = ['buildingProductPrompt', 'waterProductPrompt'];
    removeIdsBeforeElectric.forEach(id => { const el = document.getElementById(id); if (el) el.remove(); });

    // Prepare product button + estimator inside a single container so ordering is stable
    const containerId = 'productPromptContainer';
    // Remove existing container if any
    const existingContainer = document.getElementById(containerId);
    if (existingContainer) existingContainer.remove();

        const billPromptId = 'electricBillPrompt';
        const promptHtml = `
            <div id="${billPromptId}" style="margin-top:1.25rem; padding:1rem; background:#fff; border:1px dashed #d0eeda; border-radius:8px;">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <div style="font-weight:600;">Want an approximate electricity bill?</div>
                    <div style="font-size:0.9rem; color:#666;">(Rates are samples — Torrent Power)</div>
                </div>
                <div style="margin-top:0.6rem; display:flex; gap:0.75rem; align-items:center;">
                    <label><input type="radio" name="eb_mode" value="enter" checked> Enter units</label>
                    <label><input type="radio" name="eb_mode" value="use_calc"> Use calculated kWh (${Number(totalKwh.toFixed(2))} units)</label>
                </div>
                <div style="margin-top:0.6rem; display:flex; gap:0.75rem; align-items:center;">
                    <input id="eb_units_input" type="number" min="0" placeholder="Monthly units" style="width:140px; padding:6px;">
                    <input id="eb_rate_input" type="number" min="0" step="0.01" value="5.00" style="width:120px; padding:6px;"> <span style="color:#666; font-size:0.9rem;">₹/unit (custom)</span>
                    <button id="eb_estimate_btn" class="btn btn-secondary" style="margin-left:0.5rem;">Estimate</button>
                </div>
                <div id="eb_result" style="margin-top:0.6rem; font-weight:600;"></div>
                <div style="margin-top:0.5rem; font-size:0.85em; color:#666;">Actual tariffs vary by discom and taxes.</div>
            </div>
        `;

        // Build container with product button first, then estimator inside it to ensure correct visual order
        const container = document.createElement('div');
        container.id = containerId;
        container.style.marginTop = '1rem';
        container.innerHTML = `
            <div style="margin-bottom:0.6rem;">
                <a href="products.html?cat=electricity" class="btn btn-primary">View related products</a>
            </div>
            ${promptHtml}
        `;

        // Insert container after the results section
        if (resultsContainer && resultsContainer.parentNode) {
            resultsContainer.parentNode.insertBefore(container, resultsContainer.nextSibling);
        }

        // Wire estimator behaviour
        const eb_units_input = document.getElementById('eb_units_input');
        const eb_rate_input = document.getElementById('eb_rate_input');
        const eb_estimate_btn = document.getElementById('eb_estimate_btn');
        const eb_result = document.getElementById('eb_result');

        function slabCalc(units) {
            units = Math.max(0, Math.round(units));
            // Apply flat bracket rate based on total consumption (not progressive)
            let rate = 3.20;
            if (units <= 50) rate = 3.20;
            else if (units <= 100) rate = 3.95;
            else rate = 5.00;
            const subtotal = units * rate;
            const fppa_rate = 3.81; // FPPA per unit/kWh
            const fppa = units * fppa_rate;
            const fixed_charge = 50; // fixed charge
            const total = subtotal + fppa + fixed_charge;
            return { units, rate, subtotal, fppa_rate, fppa, fixed_charge, total };
        }

        eb_estimate_btn.addEventListener('click', () => {
            const mode = document.querySelector("input[name='eb_mode']:checked").value;
            let units = 0;
            if (mode === 'use_calc') units = Number(totalKwh.toFixed(2));
            else units = parseFloat(eb_units_input.value) || 0;

            // If user didn't enter units, assume 1 unit as requested
            let note = '';
            if (!units || units <= 0) { units = 1; note = ' (defaulted to 1 unit)'; }

            const customRate = parseFloat(eb_rate_input.value) || 0;
            let breakdown;
            if (customRate > 0) {
                const subtotal = units * customRate;
                const fppa_rate = 3.81;
                const fppa = units * fppa_rate;
                const fixed_charge = 50;
                const total = subtotal + fppa + fixed_charge;
                breakdown = { units, rate: customRate, subtotal, fppa_rate, fppa, fixed_charge, total };
            } else {
                breakdown = slabCalc(units);
            }

            eb_result.innerHTML = `Estimated monthly bill: ₹ ${breakdown.total.toFixed(2)} for ${breakdown.units} units${note}
                <div style="margin-top:6px; font-weight:normal; font-size:0.95rem;">
                    Breakdown:<br>
                    Units (${breakdown.units}) × Rate ₹${breakdown.rate.toFixed(2)} = ₹${breakdown.subtotal.toFixed(2)}<br>
                    FPPA (${breakdown.fppa_rate.toFixed(2)}/unit) = ₹${breakdown.fppa.toFixed(2)}<br>
                    Fixed charge = ₹${breakdown.fixed_charge.toFixed(2)}<br>
    ...
    function getEfficiencyTips(totalKwh, results) {
        ...
        while (tipsArr.length < 4) {
            tipsArr.push('Turn off devices when not in use.');
