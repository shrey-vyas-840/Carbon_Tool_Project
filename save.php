...
$host = 'localhost';
$db   = 'users'; // Make sure this matches your MySQL database name
...
$stmt = $pdo->prepare('INSERT INTO emission (appliance, units, watt, hours, kwh, co2) VALUES (?, ?, ?, ?, ?, ?)');
$success = true;
foreach ($data as $row) {
    ...
    if (isset($row['appliance'], $row['units'], $row['watt'], $row['hours'], $row['kwh'], $row['co2'])) {
        ...
        file_put_contents(__DIR__ . '/debug_log.txt', "Skipped row (not for emission table): " . print_r($row, true) . "\n", FILE_APPEND);
    }
}
